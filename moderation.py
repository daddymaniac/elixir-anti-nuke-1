...
class moderation(commands.Cog):
    ...
    async def kick(self, ctx,  user: discord.Member, *, reason="No reason provided"):   
        if ctx.author.top_role > user.top_role or ctx.author == ctx.guild.owner:
            ...
            kick = discord.Embed(title=f":boom: User Kicked:", description=f"**{user}** Was Just Kicked")
    ...
    async def kick_error(self, ctx, error):
        if isinstance(error, (commands.MissingPermissions)):
            embed = discord.Embed(title="*You Do Not Have `Kick Permissions` To Use This Command!*")
            await ctx.send(embed=embed)
        if isinstance(error, commands.MissingRequiredArgument):
            embed = discord.Embed(title="*You Must `Mention` A User To Kick!*")
            await ctx.send(embed=embed)
    ...
    async def ban(self, ctx,  user: discord.Member, *, reason="No reason provided"):   
        if ctx.author.top_role > user.top_role or ctx.author == ctx.guild.owner:
            ...
            ban = discord.Embed(title=f":boom: User Banned:", description=f"**{user}** Was Just Banned")
    ...
    async def purge_error(self, ctx, error):
        if isinstance(error, (commands.MissingPermissions)):
            embed = discord.Embed(title="*You Do Not Have ``Permissions`` to purge!*")
            await ctx.send(embed=embed)
        if isinstance(error, commands.MissingRequiredArgument):
            embed = discord.Embed(title="*You Must `Provice` An amount of messages to purge!*")
            await ctx.send(embed=embed)
    ...
    async def nuke(self, ctx, channel: discord.TextChannel = None):
        embed = discord.Embed(description=f"**:boom: Boom!**")
    ...
    async def mute(self, ctx, member: discord.Member, reason=None):
        if not member:
            ...
            return
        await ctx.author.guild.create_role(name="protection-mute")
